...
trap
{
    ...
    $agentName = $agentName + (Get-Random)
    $retryCount = 3
    $retries = 1
    do
    {
    ...
    &start cmd.exe "/k $agentExePath /configure /RunningAsService /login:$vstsUser,$vstsUserPassword /serverUrl:$serverUrl ""/WindowsServiceLogonAccount:NT AUTHORITY\System"" /WindowsServiceLogonPassword /WindowsServiceDisplayName:""$serviceDisplayName"" /name:""$agentName"" /poolname:""$poolname"" /WorkFolder:$WorkFolder /StartMode:Automatic /force /NoPrompt &exit"
